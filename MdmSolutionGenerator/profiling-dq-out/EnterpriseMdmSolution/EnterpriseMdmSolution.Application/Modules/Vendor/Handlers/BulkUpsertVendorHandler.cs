...
public sealed class BulkUpsertVendorHandler(
    IRepository<BulkOperationJob> jobRepository,
    IRepository<BulkOperationItem> itemRepository)
    : IRequestHandler<BulkUpsertVendorCommand, BulkVendorJobDto>
{
    public async Task<BulkVendorJobDto> Handle(BulkUpsertVendorCommand request, CancellationToken cancellationToken)
    {
    ...
    private static BulkVendorJobDto MapBulkJob(BulkOperationJob job, IReadOnlyList<BulkOperationItem> items)
        => new()
        {
            JobId = job.JobId,

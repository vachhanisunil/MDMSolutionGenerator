...
public interface ICustomerRunService
{
    ...
    Task<IReadOnlyList<CustomerProfilingDrilldownDto>> GetProfilingDrilldownAsync(Guid runId, Guid summaryId, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<CustomerRuleSummaryDto>> GetRuleSummaryAsync(Guid runId, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<CustomerRuleDrilldownDto>> GetRuleDrilldownAsync(Guid runId, Guid resultId, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<CustomerDuplicateDrilldownDto>> GetDuplicateDrilldownAsync(Guid runId, Guid resultId, CancellationToken cancellationToken = default);
}
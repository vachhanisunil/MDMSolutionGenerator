...
public sealed class BulkDeleteCustomerHandler(
    IRepository<BulkOperationJob> jobRepository,
    IRepository<BulkOperationItem> itemRepository)
    : IRequestHandler<BulkDeleteCustomerCommand, BulkCustomerJobDto>
{
    public async Task<BulkCustomerJobDto> Handle(BulkDeleteCustomerCommand request, CancellationToken cancellationToken)
    {

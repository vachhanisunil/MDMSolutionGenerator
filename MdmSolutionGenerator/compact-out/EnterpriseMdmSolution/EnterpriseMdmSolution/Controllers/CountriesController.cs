...
public sealed class CountriesController(GenericCrudService<Country, CountryDto, CreateCountryDto, UpdateCountryDto, int> service) : ControllerBase
{
    [HttpGet("{id}")]
    ...
    [HttpPost("search")]
    public async Task<ActionResult<PagedResult<CountryDto>>> Search(SearchCountryDto search, CancellationToken cancellationToken)

...
public sealed class CustomersController(GenericCrudService<Customer, CustomerDto, CreateCustomerDto, UpdateCustomerDto, int> service) : ControllerBase
{
    [HttpGet("{id}")]
    ...
    [HttpPost("search")]
    public async Task<ActionResult<PagedResult<CustomerDto>>> Search(SearchCustomerDto search, CancellationToken cancellationToken)

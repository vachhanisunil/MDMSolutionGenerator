...
public sealed class CustomerTaxsController(GenericCrudService<CustomerTax, CustomerTaxDto, CreateCustomerTaxDto, UpdateCustomerTaxDto, int> service) : ControllerBase
{
    [HttpGet("{id}")]
    ...
    [HttpPost("search")]
    public async Task<ActionResult<PagedResult<CustomerTaxDto>>> Search(SearchCustomerTaxDto search, CancellationToken cancellationToken)
